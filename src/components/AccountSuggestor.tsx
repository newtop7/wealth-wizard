...
export default function AccountSuggestor() {
  ...
  const handleGetAiAdvice = async () => {
    setLoadingAi(true);
    let context = "I am looking for a bank account.";
    if (filter !== 'All') context += ` I prefer a '${filter}' account.`;
    if (selectedTag !== 'All') context += ` I am interested in an account for '${selectedTag}'.`;
    
    const prompt = `Based on these bank accounts: ${JSON.stringify(filteredAccounts.length > 0 ? filteredAccounts : accounts)}, ${context} Which one is best for me? Explain why and highlight its key benefits.`;
