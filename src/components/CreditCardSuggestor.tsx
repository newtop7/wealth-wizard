...
export default function CreditCardSuggestor() {
  ...
  const handleGetAiAdvice = async () => {
    setLoadingAi(true);
    let context = "I am looking for a credit card.";
    if (filter !== 'All') context += ` I prefer a '${filter}' card.`;
    if (selectedCategory !== 'All') context += ` My main spending category is '${selectedCategory}'.`;
    if (selectedNetwork !== 'All') context += ` I prefer the '${selectedNetwork}' network.`;
    
    const prompt = `Based on these credit cards: ${JSON.stringify(filteredCards.length > 0 ? filteredCards : creditCards)}, ${context} Which one is best for me? Explain why and highlight its key benefits.`;
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        >
          <Zap size={20} />
          {loadingAi ? 'Consulting Wizard...' : "Wizard's Choice ✨"}
        </button>
      </div>
